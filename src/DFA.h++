...
public:
	...
	read(state_t q, II begin, II end) const {
		...
		for (;begin!=end; ++begin) {
			...
			if (transitionEntry == mDelta.end()) { return boost::none; }
			
			const auto it_successor = transitionEntry->second.find(*begin);
			if (it_successor == transitionEntry->second.end()) { return boost::none; }
	...
	read_infra(state_t q, II begin, II end) const {
		...
		for (;begin!=end; ++begin) {
			...
			const auto transitionEntry = mDelta.find(q);
			if (transitionEntry == mDelta.end()) { return {}; }
			...
			if (it_successor == transitionMap.end()) { return {}; }
