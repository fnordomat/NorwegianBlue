...
namespace RegEx {
		...
		if (r.type() == typeid(nil)) return r;
			...
			for (const auto& s: contents) {
				...
				if (ss.type() == typeid(assocOp<concat>)) {
					for (const regex& t : boost::get<assocOp<concat> >(ss).mContents) {
						...
						if (tt.type() == typeid(nil)) { return nil(); }
						if (tt.type() == typeid(epsilon)) { continue; }
						vec.emplace_back(tt);
					}
				} else {
					if (ss.type() == typeid(nil)) { return nil(); }
					if (ss.type() == typeid(epsilon)) { continue; }
					vec.emplace_back(ss);
				}
			}

			if (vec.size() == 0) { return epsilon(); }
			...
			for (const auto& s: contents) {
				...
				if (ss.type() == typeid(assocOp<choice>)) {
					for (const regex& t : boost::get<assocOp<concat> >(ss).mContents) {
						RegEx::regex tt = simplify(t); // snbn
						if (tt.type() == typeid(nil)) { continue; }
						set.emplace(tt);
					}
				} else {
					if (ss.type() == typeid(nil)) { continue; }
					set.emplace(ss);
				}
			}
			...
			return result; // simplify(result); // nope, no infinite loops
		}
		throw std::exception();
	}	
	...
	std::unordered_set<regex> epsilonReachable(const regex& r) {
		...
		else if (r.type() == typeid(epsilon)) { return {epsilon()}; }
		else if (r.type() == typeid(Literal)) { return {}; }
		else if (r.type() == typeid(unaryOp<kleene>))
		{
	...
	bool RegexComparator::operator()(const regex& a, const regex& b) const {
		...
		if (a.type() == typeid(assocOp<concat>)) {
			...
			if (vecA.size() < vecB.size()) { return true; }
			if (vecB.size() < vecA.size()) { return false; }
...
boost::optional<RegEx::regex> parseRegEx(const std::string& input) {
    ...
    if (input == "") { return boost::make_optional(RegEx::regex(RegEx::epsilon())); }
	...
	literalp = ~char_("\\\\()|*") | escliteralp;
	kleenep = (emptyparenp | literalp | lit('(') >> nakedregexp >> lit(')')) >> +lit('*');
	xchoicep = concatp | delimregexp;
//	concatp = +delimregexp >> delimregexp;
	concatp = +delimregexp;
//	choicep = +(xchoicep >> lit('|')) >> xchoicep;
...
/*    std::cout << "remaining: " << remaining << "/" << input.size() << ", status: " << status << std::endl;
	...
	if (!status) { return boost::none; }
	if (remaining > 0) { return boost::none; } // incomplete parse
...
std::shared_ptr<DFA<char> > makeDFAfromRegEx(const std::string& input) {
	...
	if (!maybeRex) { return std::shared_ptr<DFA<char> >(nullptr); }
